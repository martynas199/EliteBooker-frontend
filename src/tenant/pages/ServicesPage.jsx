...
export default function ServicesPage() {
    ...
    async function loadData() {
      try {
        ...
        servicesData.forEach((service, index) => {
          const specialistId = service.primaryBeauticianId?._id || service.primaryBeauticianId || service.specialistId?._id || service.specialistId;
          const specialistName = service.primaryBeauticianId?.name || service.specialistId?.name || 'N/A';
          console.log(`[Service ${index + 1}] "${service.name}" - Specialist: ${specialistName} (${specialistId})`);
        });
  ...
  // Check if any filtered services have specialists without Stripe connected
  // AND without active no-fee subscription (which allows payments without Stripe)
  const hasDisconnectedSpecialists = filteredServices.some((service) => {
    const specialist = service.primaryBeauticianId || service.specialistId;
    ...
    // Only consider as "disconnected" if no Stripe AND no no-fee subscription
    return specialist.stripeStatus !== "connected" && !hasNoFeeSubscription;
  });
  ...
  return (
    <>
      ...
      <PageTransition>
        ...
        {hasDisconnectedSpecialists && filteredServices.length > 0 && (
          <motion.div
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-amber-800 mb-3">
                    Some specialists are currently unable to accept online
                    payments. You can still book their services, but payment
                    will need to be arranged directly with them.
                  </p>
                  <p className="text-sm text-amber-700">
                    Services with available online payment will show a payment
                    option at checkout.
                  </p>
                </div>
              </div>
